...
//	}
//	
//	func uploadTransfer(amountToSend amount: Double, withComment comment: String, fromUID sender: String, toUID receiver: String, timestamp: String, sendComplete: @escaping (_ status: Bool) -> ()) {
//		REF_TRANSFERS.childByAutoId().updateChildValues(["senderId": sender, "receiverId": receiver, "amount": amount, "comment": comment])
//		sendComplete(true)
//	}
//	
//	func getAllUserTransfers(for id: String?, handler: @escaping (_ transfers: [TransferDetails]) -> ()) {
//		var transferSnapshot = [DataSnapshot]()
//		var transferArray = [TransferDetails]()
//		
//		if let userId = id {
//			REF_TRANSFERS.queryOrdered(byChild: "senderId").queryEqual(toValue: userId).observeSingleEvent(of: .value) { (feedTransferSnapshot) in
//				guard let feedTransferSnapshot = feedTransferSnapshot.children.allObjects as? [DataSnapshot] else { return }
//				transferSnapshot = feedTransferSnapshot
//			}
//		} else {
...
//		}
//		
//		for transfer in transferSnapshot {
//			let tagName = transfer.childSnapshot(forPath: "tagName").value as! String
//			let amount = transfer.childSnapshot(forPath: "amount").value as! Double
